...
# Symptom and disease dictionary (provided in sample code)
...
symptoms_selected = st.multiselect('Select your symptoms', list(symptom_dict.keys()))

if st.button('Predict Disease'):
    if symptoms_selected:
        predicted_disease = get_predicted_value(symptoms_selected)
        st.write(f"Predicted Disease: {predicted_disease}")
        
        # Fetch details related to the disease
        ...
        for i, medication in enumerate(med):
            st.write(f"{i+1}. {medication}")

        st.subheader('Workout:')
        for i, workout in enumerate(work):
